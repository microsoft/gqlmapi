...
using modified_type_t = typename service::ModifiedArgument<T>::ArgumentTraits<T, Modifiers...>::type;

template <typename T, service::TypeModifier... Modifiers>
std::optional<modified_type_t<T, Modifiers...>> GetFieldDirectiveArgument(const std::string& directiveName, const std::string& argumentName, const response::Value& fieldDirectives)
{
	auto directive = service::ModifiedArgument<response::Value>::find(directiveName, fieldDirectives);
	...
	return std::make_optional(service::ModifiedArgument<T>::require<Modifiers...>(argumentName, directive.first));
}

} // namespace

TableDirectives::TableDirectives(const std::shared_ptr<const Store>& store, const response::Value& fieldDirectives) noexcept
	: m_store{ store }
	, m_columns{ GetFieldDirectiveArgument<Column, service::TypeModifier::List>("columns", "ids", fieldDirectives) }
	, m_orderBy{ GetFieldDirectiveArgument<Order, service::TypeModifier::List>("orderBy", "sorts", fieldDirectives) }
	, m_seek{ GetFieldDirectiveArgument<response::IdType, service::TypeModifier::Nullable>("seek", "id", fieldDirectives) }
	, m_offset{ GetFieldDirectiveArgument<response::IntType>("offset", "count", fieldDirectives) }
	, m_take{ GetFieldDirectiveArgument<response::IntType>("take", "count", fieldDirectives) }
{
	if (m_store
		&& m_columns && !m_columns->empty()
		&& m_orderBy && !m_orderBy->empty())
	{
		// Pre-resolve all of the named properties in the columns and orderBy directives together in a single call
		std::vector<PropIdInput> propIds;

		propIds.reserve(m_columns->size() + m_orderBy->size());
		std::transform(m_columns->cbegin(), m_columns->cend(), std::back_insert_iterator(propIds),
			[](const auto& column) noexcept
		{
			return column.property;
		});
		std::transform(m_orderBy->cbegin(), m_orderBy->cend(), std::back_insert_iterator(propIds),
...
rowset_ptr TableDirectives::read(IMAPITable* pTable, mapi_ptr<SPropTagArray>&& defaultColumns, mapi_ptr<SSortOrderSet>&& defaultOrder) const
{
	rowset_ptr result;
	const auto defaultColumnCount = static_cast<size_t>(defaultColumns ? defaultColumns->cValues : 0);
...
mapi_ptr<SPropTagArray> TableDirectives::columns(mapi_ptr<SPropTagArray>&& defaultColumns) const
{
	...
	if (m_columns && !m_columns->empty())
	{
		...
		const size_t defaultCount = result
			? static_cast<size_t>(result->cValues)
			: 0;

		CORt(::MAPIAllocateBuffer(CbNewSPropTagArray(defaultCount + m_columns->size()), reinterpret_cast<void**>(&out_ptr{ mergedColumns })));
		...
		std::transform(m_columns->cbegin(), m_columns->cend(), propIds.begin(),
			[](const auto& column) noexcept
		{
			return column.property;
		});
		...
		else
		{
			...
			std::transform(m_columns->cbegin(), m_columns->cend(), resolved.begin(),
				[](const Column& column)
			{
...
mapi_ptr<SSortOrderSet> TableDirectives::orderBy(mapi_ptr<SSortOrderSet>&& defaultOrder) const
{
	...
	if (m_orderBy && !m_orderBy->empty())
	{
		...
		std::transform(m_orderBy->cbegin(), m_orderBy->cend(), propIds.begin(),
			[](const auto& order) noexcept
		{
			return order.property;
		});
		...
		else
		{
			...
			std::transform(m_orderBy->cbegin(), m_orderBy->cend(), resolved.begin(),
				[](const Order& order)
			{
				// Can't use named properties without a store to call GetIDsFromNames
		...
		CORt(::MAPIAllocateBuffer(CbNewSSortOrderSet(resolved.size()), reinterpret_cast<void**>(&out_ptr{ result })));
		CFRt(result != nullptr);
		...
		for (size_t i = 0; i < resolved.size(); ++i)
		{
			...
			result->aSort[i].ulOrder = m_orderBy->at(i).descending ? TABLE_SORT_DESCEND : TABLE_SORT_ASCEND;
		}
	}
...
mapi_ptr<SRestriction> TableDirectives::seek() const
{
	...
	if (m_seek && *m_seek)
	{
		...
		LPBYTE pb = nullptr;
		const ULONG cbAlloc = static_cast<ULONG>(sizeof(*result) + sizeof(*pval) + (*m_seek)->size());
...
{
	return m_seek
		? (*m_seek ? BOOKMARK_CURRENT : BOOKMARK_END)
		: BOOKMARK_BEGINNING;
}
