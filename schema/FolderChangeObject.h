...
class [[nodiscard("unnecessary construction")]] FolderChange final
...
private:
	...
	explicit FolderChange(std::unique_ptr<const Concept> pimpl) noexcept;
...
public:
	template <class T>
	explicit FolderChange(std::shared_ptr<T> pimpl) noexcept
		: FolderChange { std::unique_ptr<const Concept> { std::make_unique<Model<T>>(std::move(pimpl)) } }
	{
