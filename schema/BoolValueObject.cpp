...
std::future<service::ResolverResult> BoolValue::resolveValue(service::ResolverParams&& params)
{
...
std::future<service::ResolverResult> BoolValue::resolve_typename(service::ResolverParams&& params)
{
...
void AddBoolValueDetails(std::shared_ptr<schema::ObjectType> typeBoolValue, const std::shared_ptr<schema::Schema>& schema)
{
	typeBoolValue->AddFields({
		schema::Field::Make(R"gql(value)gql"sv, R"md()md"sv, std::nullopt, schema->WrapType(introspection::TypeKind::NON_NULL, schema->LookupType("Boolean")))
	});
}
