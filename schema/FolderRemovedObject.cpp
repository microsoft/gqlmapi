...
FolderRemoved::FolderRemoved(std::unique_ptr<const Concept> pimpl) noexcept
...
{
	return service::Result<std::string>::convert(std::string{ R"gql(FolderRemoved)gql" }, std::move(params));
}
