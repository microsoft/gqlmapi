...
std::future<service::ResolverResult> FolderRemoved::resolveIndex(service::ResolverParams&& params)
{
...
std::future<service::ResolverResult> FolderRemoved::resolveRemoved(service::ResolverParams&& params)
{
	std::unique_lock resolverLock(_resolverMutex);
	auto directives = std::move(params.fieldDirectives);
...
std::future<service::ResolverResult> FolderRemoved::resolve_typename(service::ResolverParams&& params)
{
...
void AddFolderRemovedDetails(std::shared_ptr<schema::ObjectType> typeFolderRemoved, const std::shared_ptr<schema::Schema>& schema)
{
	typeFolderRemoved->AddFields({
		...
		schema::Field::Make(R"gql(removed)gql"sv, R"md(ID of the `Folder` that was removed)md"sv, std::nullopt, schema->WrapType(introspection::TypeKind::NON_NULL, schema->LookupType("ID")))
	});
}
