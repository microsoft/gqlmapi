...
class [[nodiscard("unnecessary construction")]] GuidValue final
...
private:
		...
		: Concept
	{
		...
		[[nodiscard("unnecessary call")]] service::AwaitableScalar<response::Value> getValue(service::FieldParams&& params) const override
		{
			...
			{
				throw service::unimplemented_method(R"ex(GuidValue::getValue)ex");
			}
		}
	...
	explicit GuidValue(std::unique_ptr<const Concept> pimpl) noexcept;
...
public:
	template <class T>
	explicit GuidValue(std::shared_ptr<T> pimpl) noexcept
		: GuidValue { std::unique_ptr<const Concept> { std::make_unique<Model<T>>(std::move(pimpl)) } }
	{
	}

	[[nodiscard("unnecessary call")]] static constexpr std::string_view getObjectType() noexcept
	{
		return { R"gql(GuidValue)gql" };
	}
};
