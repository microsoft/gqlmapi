...
class [[nodiscard("unnecessary construction")]] StreamValue final
...
private:
		...
		: Concept
	{
		...
		[[nodiscard("unnecessary call")]] service::AwaitableScalar<response::Value> getValue(service::FieldParams&& params) const override
		{
			...
			{
				throw service::unimplemented_method(R"ex(StreamValue::getValue)ex");
			}
		}
	...
	explicit StreamValue(std::unique_ptr<const Concept> pimpl) noexcept;
...
public:
	template <class T>
	explicit StreamValue(std::shared_ptr<T> pimpl) noexcept
		: StreamValue { std::unique_ptr<const Concept> { std::make_unique<Model<T>>(std::move(pimpl)) } }
	{
	}

	[[nodiscard("unnecessary call")]] static constexpr std::string_view getObjectType() noexcept
	{
		return { R"gql(StreamValue)gql" };
	}
};
