...
#ifndef ITEMCHANGEOBJECT_H
#define ITEMCHANGEOBJECT_H

#include "MAPISchema.h"

namespace graphql::mapi::object {

class [[nodiscard("unnecessary construction")]] ItemChange final
...
private:
	...
	explicit ItemChange(std::unique_ptr<const Concept> pimpl) noexcept;
...
public:
	template <class T>
	explicit ItemChange(std::shared_ptr<T> pimpl) noexcept
		: ItemChange { std::unique_ptr<const Concept> { std::make_unique<Model<T>>(std::move(pimpl)) } }
	{
		static_assert(T::template implements<ItemChange>(), "ItemChange is not implemented");
	}
};

} // namespace graphql::mapi::object

#endif // ITEMCHANGEOBJECT_H
