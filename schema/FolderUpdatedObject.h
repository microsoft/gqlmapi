...
class [[nodiscard("unnecessary construction")]] FolderUpdated final
...
private:
	...
	struct [[nodiscard("unnecessary construction")]] Concept
	{
		...
		[[nodiscard("unnecessary call")]] virtual service::AwaitableObject<std::shared_ptr<Folder>> getUpdated(service::FieldParams&& params) const = 0;
	};
		...
		: Concept
	{
		...
		[[nodiscard("unnecessary call")]] service::AwaitableScalar<int> getIndex(service::FieldParams&& params) const override
		{
			...
			{
				throw service::unimplemented_method(R"ex(FolderUpdated::getIndex)ex");
			}
		}

		[[nodiscard("unnecessary call")]] service::AwaitableObject<std::shared_ptr<Folder>> getUpdated(service::FieldParams&& params) const override
		{
			...
			{
				throw service::unimplemented_method(R"ex(FolderUpdated::getUpdated)ex");
			}
		}
	...
	explicit FolderUpdated(std::unique_ptr<const Concept> pimpl) noexcept;
...
public:
	template <class T>
	explicit FolderUpdated(std::shared_ptr<T> pimpl) noexcept
		: FolderUpdated { std::unique_ptr<const Concept> { std::make_unique<Model<T>>(std::move(pimpl)) } }
	{
	}

	[[nodiscard("unnecessary call")]] static constexpr std::string_view getObjectType() noexcept
	{
		return { R"gql(FolderUpdated)gql" };
	}
};
